...
import {
  BridgeExecutionSubmittedEventObject,
  BridgeRequestWithdrawnEventObject,
  BridgeRequestedEventObject
} from "../typechain-types/contracts/BridgeWhenCheap.sol/BridgeWhenCheap";
import {
  BridgeRequest,
  GetEventByName,
  allRequestsEmpty,
...
describe("Deposits", function () {
  ...
  for (const tc of testPermutations()) {
    it(
      "end2end workflow " +
        (tc.expectDepositFailure && tc.expectExecFailure
          ? "success"
          : "failure") +
        " " +
        tc.desc,
      async () => {
        ...
        const {
          ...
          wantedL1GasFee,
          followUp,
          expectDepositFailure,
          expectExecFailure
        } = tc;

        // Ensure, that the previous nonces are filled.
        const balanceBeforeSetup = await sender.getBalance();
        for (let n = 0; BigNumber.from(n).lt(nonce); n++) {
          expect(
            await bwc
              .connect(sender)
              .deposit(nativeEther, 0, sender.address, 1, 0, {
                value: serviceFee + 100
              })
          );
        }
        const etherUsedUpForSetup = balanceBeforeSetup.sub(
        ...
        const actualBridgeRequestedEvent =
          await GetEventByName<BridgeRequestedEventObject>(
            "BridgeRequested",
            deposit
          );
        expect(actualBridgeRequestedEvent.nonce).to.equal(nonce);
        expect(actualBridgeRequestedEvent.request).to.deep.equal(
          toStructOutput(expectedRequest)
        );
        // ====================== ^^^^^^ getting this all right was difficult !! ^^^^^ ================================

        expect(await sender.getBalance()).to.equal(
          initialNativeBalance
            .sub(etherUsedUpForSetup)
            .sub(nativeEtherAmount)
            .sub(await totalPaidGasFeesOfTx(deposit))
        );

        if (tokenTransfer) {
          expect(await token.balanceOf(sender.address)).to.equal(
            BigNumber.from(initialAccountTokenBalance).sub(tokenAmount)
          );
        }

        expect(await bwc.collectedServiceFeeExcludingGas()).equal(
          BigNumber.from(nonce) // setup
            .add(1) // testcase
            .mul(serviceFee - l2GasfeeDeposit)
        );

        const request: BridgeRequest = await bwc.pendingRequests(
          sender.address,
          nonce
        );
        ...
        expect(request.l2execGasFeeDeposit).equal(
          expectedRequest.l2execGasFeeDeposit
        );
        expect(request.wantedL1GasPrice).equal(
          expectedRequest.wantedL1GasPrice
        );

        expect(
          isEmpty(
            await bwc.pendingRequests(
              sender.address,
              BigNumber.from(nonce).add(1)
            )
          )
        ).to.be.true;
        ...
        if (followUp === "withdraw") {
          ...
          const withdraw = await bwc.connect(sender).withdraw(nonce);

          // Check event correctly emitted
          const actualBridgeRequestWithdrawnEvent =
            await GetEventByName<BridgeRequestWithdrawnEventObject>(
              "BridgeRequestWithdrawn",
              withdraw
            );
          expect(actualBridgeRequestWithdrawnEvent.nonce).to.equal(nonce);
          expect(actualBridgeRequestWithdrawnEvent.request).to.deep.equal(
            toStructOutput(expectedRequest)
          );
          ...
          const expectedTokenBalance = requestorTokenBalance.add(
            tokenTransfer ? request.amount : 0
          );

          expect(await sender.getBalance()).to.equal(
            expectedNativeEtherBalance
          );
          expect(await token.balanceOf(sender.address)).to.equal(
            expectedTokenBalance
          );

          expect(isEmpty(await bwc.pendingRequests(sender.address, nonce))).to
            .be.true;
        } else {
          // ======================== execute request

          const ownerBalanceBeforeExec = await owner.getBalance();
          const nativeEtherSent = tokenTransfer
            ? BigNumber.from(0)
            : request.amount;
          ...
          const execP = bwc.executeRequest(
            sender.address,
            nonce,
            followUp.bonderFee,
            0
          ...
          await expect(execP).to.changeEtherBalances(
            ...
            [
              nativeEtherSent.add(l2GasfeeDeposit).mul(-1),
              nativeEtherSent,
              l2GasfeeDeposit
            ]
          );
          ...
          const actualBridgeExecutionSubmittedEvent =
            await GetEventByName<BridgeExecutionSubmittedEventObject>(
              "BridgeExecutionSubmitted",
              exec
            );
          expect(actualBridgeExecutionSubmittedEvent.nonce).to.equal(nonce);
          expect(actualBridgeExecutionSubmittedEvent.request).to.deep.equal(
            toStructOutput(expectedRequest)
          );
          ...
          expect(await owner.getBalance()).to.equal(
            ownerBalanceBeforeExec
              .add(l2GasfeeDeposit)
              .sub(await totalPaidGasFeesOfTx(exec))
          );

          // requestor balances don't change.
          expect(await sender.getBalance()).equal(requestorNativeBalance);
          expect(await token.balanceOf(sender.address)).equal(
            requestorTokenBalance
          );

          // =========== owner withdraws fees
          const amountToWithdraw = BigNumber.from(nonce) // setup
            .add(1) // testcase
            .mul(heldFeePerRequest);
          expect(await bwc.collectedServiceFeeExcludingGas()).to.equal(
            amountToWithdraw
          );
          await expect(
            bwc.ownerWithdraw(amountToWithdraw.add(1))
          ).to.be.revertedWith(
            /Cannot withdraw more funds than the collected non gas service fees/
          );
          expect(await bwc.ownerWithdraw(amountToWithdraw));
        }
      }
    );
  }
...
function testPermutations(): DepositTestCase[] {
  ...
  const amounts: BigNumber[] = [
    ...
    300,
    initialAccountTokenBalance,
    initialAccountTokenBalance + 1,
    parseUnits("9999", "ether")
  ].map(BigNumber.from);
    ...
    for (const amount of amounts) {
      for (const tokenTransfer of tokenTransfers) {
        ...
        for (const bonderFee of bonderFees) {
          ...
          if (tokenTransfer) {
            if (amount.gt(initialAllowance)) {
              expectDepositFailure =
                expectDepositFailure ?? /ERC20: insufficient allowance/;
            }
            if (amount.gt(initialAccountTokenBalance)) {
              expectDepositFailure =
                expectDepositFailure ??
            ...
            if (amount.lt(serviceFee)) {
              expectDepositFailure =
                expectDepositFailure ??
                /Not enough funds to pay for delayed execution/;
            }
            minOutAmount = BigNumber.from(amount)
              .sub(serviceFee)
              .sub(bonderFee);
          }
          if (minOutAmount.lt(0)) continue; // tx cannot be sent

          let expectExecFailure;
          if (amount.sub(bonderFee).lt(minOutAmount)) {
            expectExecFailure = /Bonder fee cannot exceed amount/;
          }
            ...
            for (const execRequest of [true, false]) {
              tcs.push({
                desc:
                  ...
                  tokenTransfer +
                  " wl1" +
                  wantedL1GasFee.toString() +
                  (execRequest ? " bf" + bonderFee.toString() : " wd"),
                amount,
                nonce,
                minOutAmount,
                tokenTransfer,
